...
let bvhController = null;
let bvhUrls ={
    "default": "Default.bvh",
    "sample": "sample.bvh",
    "hook": "Hook.bvh",
    "backflip": "Backflip.bvh",
    "greeting": "greeting.bvh"
};
...
async function loadBVHScenario(scenarioName) {
    if (!bvhUrls[scenarioName]) {
        console.error("No BVH URL defined for scenario.");
        return;
    }

    const url = bvhUrls[scenarioName];
    bvhController = await loadBVHAnimation(url, root, hierarchy);
    if (bvhController) {
        currentAnimation = scenarioName;
    } else {
        console.error("Failed to load BVH animation");
    }
}
...
function render(time = 0) {
    ...
    if (bvhController){
        bvhController.update(root, time);
    }
    traverse(root);
